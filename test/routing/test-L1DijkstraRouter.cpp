...
TEST_F(AL1DijkstraRouter, findsAllCandidatesForVerticalTargets)
{
	...
	dijkstra.add_target(target);
	dijkstra.run();
	EXPECT_EQ(8, dijkstra.vertices_for(target).size());
	std::set<L1RoutingGraph::vertex_descriptor> vertices,
		reference{
			routing_graph[hicann2][VLineOnHICANN(15)],  routing_graph[hicann2][VLineOnHICANN(47)],
			routing_graph[hicann2][VLineOnHICANN(79)],  routing_graph[hicann2][VLineOnHICANN(111)],
			routing_graph[hicann2][VLineOnHICANN(144)], routing_graph[hicann2][VLineOnHICANN(176)],
			routing_graph[hicann2][VLineOnHICANN(208)], routing_graph[hicann2][VLineOnHICANN(240)]};
	for (auto const& vertex : dijkstra.vertices_for(target)) {
		vertices.insert(vertex);
	}
	ASSERT_TRUE(std::equal(vertices.begin(), vertices.end(), reference.begin()));
